...
public:
  int maximumScore(vector<int>& nums, vector<int>& multipliers) {
    ...
    vector<vector<int>> dp(2, vector<int>(m + 1, -0x3f3f3f3f));

    dp[0][0] = 0;
    for (int i = 1; i <= m; ++i) {
      vector<int>& cur = dp[i & 1];
      vector<int>& pre = dp[!(i & 1)];

      cur[0] = pre[0] + multipliers[i - 1] * nums[n - i];
      cur[i] = pre[i - 1] + multipliers[i - 1] * nums[i - 1];
      for (int j = 1; j < i; ++j) {
        cur[j] = max(
          pre[j - 1] + multipliers[i - 1] * nums[j - 1],
          pre[j] + multipliers[i - 1] * nums[n - (i - j)]
        );
      }
    }

    int answer = -0x3f3f3f3f;
    vector<int>& result = dp[m & 1];
    for (int i = 0; i <= m; ++i) {
